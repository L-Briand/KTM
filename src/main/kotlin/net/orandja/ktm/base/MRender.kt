...
fun interface MRender {
     ...
     */
    fun render(provider: MProvider, token: MToken, pool: MPool, node: NodeContext, writer: (CharSequence) -> Unit)
     ...
     */
    fun render(document: MDocument, pool: MPool, context: NodeContext, writer: (CharSequence) -> Unit) {
    ...
    fun render(document: MDocument, pool: MPool, context: MContext, writer: (CharSequence) -> Unit) =
        render(document.provider, document.tokens, pool, NodeContext(context), writer)
    ...
    fun renderToString(document: MDocument, partial: MPool, context: MContext): String {
        val length = document.tokens.stop - document.tokens.start
