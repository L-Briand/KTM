...
import net.orandja.ktm.annotations.SymbolProcessorConfiguration.Companion.OPTION_ALLOW_INTERNAL_CLASS
import net.orandja.ktm.annotations.generation.AdapterToken.*
import net.orandja.ktm.annotations.visitor.VisitorResult.*
...
class TokenBuilderVisitor(private val ktmLogger: KtmLogger) :
    ...
    override fun visitTypeReference(typeReference: KSTypeReference, data: Options): VisitorResult {
        ...
        // Like visitCallableReference check for first argument being a MContext.Node
        // For callable `MContext.Node.() -> T` is the same as `(MContext.Node) -> T`
        // If there is more argument, don't handle it
        if (reference.arguments.size > 2) return Empty

        var isNodeParameter = false

        // Check for MContext.Node on the first parameter of the function
        if (reference.arguments.isNotEmpty()) {
            ...
            isNodeParameter = typeParameter.node.qualifiedNameFromReference() == "MContext.Node"
    ...
    override fun visitCallableReference(reference: KSCallableReference, data: Options): VisitorResult {
        ...
        // @KtmContext allows
        // - normal callable `() -> T`
        // - MContext.Node receiver `MContext.Node.() -> Unit`
        // - MContext.Node parameter `(MContext.Node) -> Unit`
        ...
        // Check for MContext.Node as receiver (
        val receiver = reference.receiverType?.accept(this, data)
        if (receiver != null) {
            // If it has a receiver which is not MContext.Node, it should not be handled
            if (receiver !is Classifier) return Empty
            isNodeReceiver = receiver.node.qualifiedNameFromReference() == "MContext.Node"
            if (!isNodeReceiver) return Empty
        }

        // Callable with MContext.Node in both receiver and parameter is invalid
    ...
    /** A like [visitPropertyDeclaration] without type checks for MContext.Node */
    override fun visitFunctionDeclaration(function: KSFunctionDeclaration, data: Options): VisitorResult {
        ...
        // - MContext.Node receiver `MContext.Node.foo() -> Unit`
        // - MContext.Node parameter `foo(context: MContext.Node) -> Unit`
        ...
        // Only handle when MContext.Node is the only parameter func(context: MContext.Node)
        ...
        // Function with MContext.Node in both receiver and parameter is invalid
        if (isNodeParameter && isNodeReceiver) return Empty
    ...
    private fun Sequence<KSAnnotation>.isfKtmDynamic(data: Options) = find {
    ...
    private fun KSNode.qualifiedNameFromReference(): String? {
        if (this !is KSClassifierReference) return null
        return qualifier?.accept(ClassNameVisitor(ktmLogger), referencedName()) ?: return referencedName()
    }
}